...
export interface PatternResult {
  pattern: PatternRule;
  evidence: { recordIds: string[]; summary: string };
  score: number;
}
...
export function getPatternRules(module?: string): PatternRule[] {
  return module ? RULES.filter(r => r.module === module) : [...RULES];
}

export function processPatterns(stats: Record<string, any>): PatternResult[] {
  const results: PatternResult[] = [];
  for (const rule of RULES) {
    const total = rule.tags.reduce((s, tag) => {
      const errs: any[] = stats[rule.module]?.topErrors ?? [];
      return s + errs.filter((e: any) => (e.category === tag || e.errorCategory === tag)).reduce((a: number, e: any) => a + e.count, 0);
    }, 0);
    if (total === 0) continue;
