...
export const AdaptivePlanSchema = z.object({
  ...
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine((v: string) => !isNaN(new Date(v).getTime()), 'Invalid date'),
  endDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().default(null).refine((v: string | null) => v === null || !isNaN(new Date(v).getTime()), 'Invalid date'),
 ...
 .refine((p: any) => { const ts = p.tasks || []; return p.summary.total === ts.length && p.summary.completed === ts.filter((t: any) => t.status === 'done').length && p.summary.skipped === ts.filter((t: any) => t.status === 'skipped').length; }, 'Summary must match task counts')
