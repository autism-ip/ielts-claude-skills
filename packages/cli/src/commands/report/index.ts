...
export function registerReportCommands(program: Command): void {
  ...
  report.command('weekly')
    ...
    .action(() => {
      ...
      if (!stats) { console.log('No data. Run ielts snapshot first.'); return; }
      ...
      lines.push('  Vocab: ' + (stats.vocab?.wordsReviewed ?? 0) + ' words'); lines.push('');
      ...
      lines.push('  Overall: ' + (stats.combined?.overallBand ?? 'N/A')); lines.push('');

      if (plan?.tasks) {
        const done = plan.tasks.filter((t: any) => t.status === 'done').length;
        const skipped = plan.tasks.filter((t: any) => t.status === 'skipped').length;
        lines.push('Plan: ' + done + '/' + plan.tasks.length + ' done, ' + skipped + ' skipped'); lines.push('');
      }
