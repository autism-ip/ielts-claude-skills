...
function writeFm(path, data, body) {
    const lines = ['---'];
...
function makeListeningFixtures() {
    return [
        {
...
/* ── 主入口 ── */
export function installFixtures(force) {
    if (!force && existsSync(BASE_LOCK)) {
        console.error('~/.ielts/ has real data. Use --force to overwrite.');
        return;
    }
    /* 清理旧记录，确保幂等性 */
    ...
    makeWritingFixtures().forEach((rec) => {
        ...
        const slug = rec.taskType;
        writeFm(join(BASE, 'writing', `${date}-${slug}.md`), rec, `This is a sample IELTS Writing Task ${slug === 'task1' ? 1 : slug === 'task2' ? 2 : 'General Training Letter'} essay on the topic: "${rec.topic}".\n\n[Full essay text omitted for fixture brevity.]`);
    });
    /* Reading: date-based filenames */
    makeReadingFixtures().forEach((rec) => {
        const date = rec.createdAt.slice(0, 10);
        const slug = rec.passageTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    ...
    try {
        rmSync(join(BASE, 'speaking'), { recursive: true, force: true });
    }
    catch { /* 不存在 */ }
